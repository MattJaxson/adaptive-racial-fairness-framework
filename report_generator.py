...
----------------------
Converts an audit result dict into a professional PDF report.
...
def generate_pdf_report(audit_result: dict[str, Any]) -> bytes:
    """
    Generate a PDF audit report from an audit result dict.

    Parameters
    ----------
    audit_result : dict
        The dict returned by _build_audit_report() in api/main.py.

    Returns
    -------
    bytes
        Raw PDF bytes, ready to stream or write to a file.
    """
    buffer = io.BytesIO()
    ...
    if audit_type == "standard":
        story.append(Paragraph(
            "This audit used researcher-defined fairness parameters. "
            "For a community-valid audit, supply a community_definitions.json "
    ...
    story.append(Paragraph(
        "A Disparate Impact ratio below 0.80 triggers the EEOC 4/5ths rule threshold "
        "and indicates potential discriminatory outcomes.",
        body_style,
    ))
    ...
    for i, row in enumerate(di_rows[1:], start=1):
        if "FLAGGED" in row[3]:
            di_style.append(("TEXTCOLOR", (3, i), (3, i), FLAG_RED))
            di_style.append(("FONTNAME", (3, i), (3, i), "Helvetica-Bold"))

    di_table.setStyle(TableStyle(di_style))
    ...
    rec_style = ParagraphStyle(
        "Rec",
        parent=body_style,
        backColor=colors.HexColor("#fef2f2") if flagged else colors.HexColor("#f0fdf4"),
    ...
    if provenance:
        config_data += [
            ...
            ["Input Location", provenance.get("input_location", "—")],
            ["Participants", str(provenance.get("input_participants", "—"))],
            ["Facilitator", provenance.get("facilitator", "—")],
    ...
    story.append(Paragraph(
        "Adaptive Racial Fairness Framework — adaptivefairness.org | "
        "Powered by Community-Defined Fairness Protocol v0.1",
        small_style,
    ))
