"""
Zero-Trust Test Suite — Adaptive Racial Fairness Framework
============================================================
Adversarial edge cases, boundary conditions, and correctness proofs
for the core fairness pipeline.
...
def large_df():
    ...
    # Set known approval rates by group
...
class TestRacialBiasScore:
    ...
    def test_nan_values_dropped(self):
        ...
        # 3 valid rows: White=[1], Black=[0,1] → White=1.0, Black=0.5
        assert result["group_outcomes"]["White"] == 1.0
        assert result["group_outcomes"]["Black"] == 0.5
...
class TestFairnessAudit:
    ...
    def test_disparate_impact_half_rate(self):
        ...
        # White rate = 3/4 = 0.75, Black rate = 1/4 = 0.25
        di = disparate_impact(df, "race", "outcome", "White", "Black", 1)
    ...
    def test_disparate_impact_missing_group(self):
        """Group not in dataset → returns DI=0 (no favorable outcomes found)."""
        df = pd.DataFrame({
            "race": ["White", "White"],
            "outcome": [1, 1],
        })
...
class TestFairnessReweight:
    ...
    def test_priority_groups_get_adjusted_weight(self, simple_df, community_defs_default):
        ...
        # Black rate = 0.5, White rate = 1.0
        # Favorable: w = 1.0 / 0.5 = 2.0
...
class TestCommunityInput:
    ...
    def test_build_basic(self):
        ...
        assert config["provenance"]["record_id"]  # UUID exists
        assert config["provenance"]["input_date"]  # Date exists
    ...
    def test_build_low_confidence_flag(self):
        config = build_community_config(
            priority_groups=["Black"],
            fairness_target="White",
            input_participants=5,  # Below MIN_PARTICIPANTS=10
        )
...
class TestIntegration:
    ...
    def test_threshold_changes_flagging(self, large_df):
        ...
        assert flagged_strict  # Must be True at strict threshold
    ...
    def test_unicode_group_names(self):
        ...
        df = pd.DataFrame({
            "race": ["Café", "Naïve", "Café", "Naïve"],
            "outcome": [1, 0, 1, 0],
        })
        result = calculate_racial_bias_score(df, "race", "outcome")
        assert "Café" in result["group_outcomes"]
...
class TestAdversarial:
    ...
    def test_config_threshold_boundary(self):
        ...
        # White rate = 1.0, Black rate = 0.8, DI = 0.8
        di = disparate_impact(df, "race", "outcome", "White", "Black", 1)
        assert di == 0.8
        # At threshold 0.8, DI >= threshold → NOT flagged
        assert not (di < 0.8)
